/*
 * Webster - https://github.com/rnosach/webster.git
 * author: Vilius L.
 */
...
class Base_tools_class {
	...
	events() {
		...
		document.addEventListener('touchmove', function (event) {
			_this.dragMove(event);
			if (event.target.id === "canvas_webster" && !$('.scroll').has($(event.target)).length)
	...
	set_mouse_info(event) {
		...
		if (event.target.id != 'canvas_webster' && event.target.id != 'main_wrapper') {
			//outside canvas
			this.mouse_valid = false;
		}
		else {
		...
		if (eventType === 'mousedown' || eventType === 'touchstart') {
			if ((event.target.id != 'canvas_webster' && event.target.id != 'main_wrapper') || (event.which != 1 && eventType !== 'touchstart')) {
				this.mouse_click_valid = false;
			}
			else {
	...
	get_mouse_info(event) {
		if(typeof event != "undefined" && typeof mouse_x == "undefined"){
