...
class Base_gui_class {
	...
	set_events() {
		...
		document.getElementById('canvas_webster').addEventListener('contextmenu', function (e) {
			e.preventDefault();
		}, false);
	}
	...
	check_canvas_offset() {
		...
		var canvas_el = document.getElementById('canvas_webster').getBoundingClientRect();
		this.canvas_offset.x = canvas_el.left - bodyRect.left;
		this.canvas_offset.y = canvas_el.top - bodyRect.top;
	}

	prepare_canvas() {
		var canvas = document.getElementById('canvas_webster');
		...
		this.render_canvas_background('canvas_webster');
